...
type Props = {
  events: GraduationSchedule[]; // already filtered by getTodayEvents()
  now: Date;
};
...
export default function CurrentEventCard({ events, now }: Props) {
  ...
  return (
    <div className="flex justify-center mb-16">
      {/* Animated Border & High-Highlight Deep Shadow Wrapper */}
      <div 
        className={cn(
      ...
      >
        {/* Animated Gradient Line Border (Fires only when events are active right now) */}
        {ongoingCount > 0 && (
        ...
        <div className="relative w-full rounded-[11px] overflow-hidden bg-white font-sans z-10">
            ...
            <ul className="divide-y divide-neutral-100">
              {grouped.map(({ status, items }) => (
                <div key={`group-${status}`}>
                  ...
                  {/* Event rows */}
                  {items.map((ev, i) => {
                    if (ev.id === undefined) return null; // Safe guard if id is missing
                      ...
                      <li key={ev.id} className="border-b border-neutral-100 last:border-0 bg-white">
                        ...
                        <div className={cn(
                          ...
                          <div className="px-5 pb-4 pl-[50px]">
                            {/* <p className="font-mono text-[11px] text-neutral-400 mb-2 flex items-center gap-1.5">
                              <Calendar size={12} />
          ...
          {/* Footer */}
          {/* <div className="flex items-center gap-2 px-5 py-2.5 bg-neutral-50 border-t border-neutral-100">
            <span className="font-mono text-[10px] uppercase tracking-widest text-neutral-400">graduation schedule</span>
            <span className="w-1 h-1 rounded-full bg-neutral-300" />
            <span className="font-mono text-[10px] uppercase tracking-widest text-neutral-400">
              {ongoingCount > 0 ? 'ongoing' : events.length > 0 ? 'between events' : 'idle'}
            </span>
          </div> */}
